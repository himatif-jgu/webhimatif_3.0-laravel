...
@section('content')
@php
    $cms = $landingContents ?? collect();
    $hero = $cms->get('hero-main');
    $about = $cms->get('about-main');
    $historyFaqs = ($historyEntries ?? collect())->get('faq', collect());
    $historyJourneys = ($historyEntries ?? collect())->get('journey', collect());
    $foundationItems = $foundationContents ?? collect();
    ...
    <div class="boxed_wrapper ltr">
        ...
        <section id="home" class="banner-section p_relative centred">
            ...
            <div class="auto-container">
                <div class="content-box">
                    ...
                    <h3 style="color: #fff; font-style: italic;">{{ $hero?->subtitle ?? 'Driving #TechForImpact through Digital Innovation' }}</h3>
                    <p>{!! $hero?->body ?? 'We are more than a community. We are an ecosystem for shaping digital talents who are ready to innovate, collaborate, and create tech solutions that deliver real-world impact, embodying the spirit of impactful science and technology.' !!}</p>
                    <div class="btn-box">
                        <a href="{{ $hero?->button_url ?? route('home') }}" class="theme-btn btn-one mr_20"><span>{{ $hero?->button_label ?? 'Explore Our Programs' }}</span></a>
                        ...
                        <div class="content_block_one">
                            <div class="content-box ml_80">
                                <div class="sec-title pb_20 sec-title-animation animation-style2">
                                    <span class="sub-title mb_10 title-animation">{{ $about?->eyebrow ?? 'About us' }}</span>
                                    <h2 class="title-animation">{!! $about?->title ?? 'Building the Next Generation of <span>Technologists</span>' !!}</h2>
                                </div>
                                <div class="text-box">
                                    <p>{!! $about?->body ?? 'The Informatics Engineering Student Association (HIMATIF) is the official organization for all Informatics Engineering students at Jakarta Global University. We serve as a hub for personal growth, skill development, professional networking, and creating technology that is both useful and impactful.' !!}</p>
                    ...
                    <div class="col-lg-6 col-md-12 col-sm-12 content-column">
                        <div class="content_block_three">
                            <div class="content-box mr_150">
                                ...
                                @if($historyFaqs->isNotEmpty())
                                <ul class="accordion-box">
                                    @foreach($historyFaqs as $faq)
                    ...
                    <div class="col-lg-6 col-md-12 col-sm-12 content-column">
                        <div class="content_block_two">
                            <div class="content-box ml_100">
                                ...
                                @if($historyJourneys->isNotEmpty())
                                <div class="inner-box">
                                    @foreach($historyJourneys as $journey)
                                    <div class="single-item">
                                        <span class="count-text">{{ $loop->iteration }}</span>
                                        <h3><a href="#history">{{ $journey->title }}</a></h3>
                                        {!! $journey->body !!}
                                    </div>
                                    @endforeach
                                </div>
            ...
            <div class="auto-container">
                ...
                <div class="inner-container clearfix">
                    ...
                    <div class="industries-block-one">
                        <div class="inner-box">
                            <div class="icon-box"><i data-feather="{{ $team->icon ?: 'users' }}" width="40" height="40"></i></div>
        ...
        <section class="team-section centred pt_30 pb_70">
            <div class="auto-container">
                ...
                <div class="row clearfix">
                    @foreach($leaderItems as $member)
                    @php
                        $photo = $member->photo_path ? asset('storage/' . $member->photo_path) : asset('assets/landing/images/bph/' . $loop->iteration . '.png');
                    @endphp
                    <div class="col-lg-3 col-md-6 col-sm-12 team-block">
                        <div class="team-block-one wow fadeInUp animated" data-wow-delay="{{ ($loop->index % 4) * 200 }}ms"
                            data-wow-duration="1500ms">
                            <div class="inner-box">
                                <div class="image-box">
                                    <figure class="image"><img src="{{ $photo }}" alt="{{ $member->name }}"></figure>
                                    <figure class="overlay-image"><img src="{{ $photo }}" alt="{{ $member->name }}"></figure>
                                </div>
                                <div class="lower-content">
                                    <h3><a href="{{ $member->profile_url ?: route('home') }}" @if($member->profile_url) target="_blank" rel="noopener" @endif>{{ $member->name }}</a></h3>
        ...
        <!-- process-section -->
        <section id="programs" class="process-section pt_120 pb_90">
                        ...
                        <div class="col-lg-4 col-md-12 col-sm-12 info-column">
                            <div class="info-box">
                                ...
                                @if($contactItems->isNotEmpty())
                                    ...
                                    <div class="single-item">
                                        <div class="icon-box"><img src="{{ $contact->icon_path ? asset('storage/' . $contact->icon_path) : asset('assets/landing/images/icons/icon-27.png') }}"
                                                alt="{{ $contact->label }}"></div>
                                        <h4>{{ $contact->label }}</h4>
                                        <p>
                                            @if($contact->url)
                                                <a href="{{ $contact->url }}">{!! nl2br(e($contact->value)) !!}</a>
                                            @else
